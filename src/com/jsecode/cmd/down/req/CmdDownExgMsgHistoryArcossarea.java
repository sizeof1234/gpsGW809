 ...
 * 
 * 
 * 项目名称：gpsGW809 类名称：CmdDownExgMsgHistoryArcossarea 类描述：
 ***************************** 
 * 4.5.3.2.3车辆定位信息交换补发消息 子业务类型标识：DOWN_EXG_MSG_HISTORY_ARCOSSAREA
 * 描述：本业务在DOWN_EXG_MSG_APPLY_HISGNSSDATA_ACK应答成功后，立即开始交换
 * 如果申请失败，则不进行数据转发。本条消息下级平台无需应答
 ***************************** 
 * 创建人：zhaorg 创建时间：2014-2-17 下午01:52:42 修改人： 修改时间： 修改备注：
 * 
 * @version
 * 
 * 
...
public class CmdDownExgMsgHistoryArcossarea extends CmdHeadSubBizWithCar {
	...
	protected void fillCmdSubBizData(ChannelBuffer channelBuffer) {
		channelBuffer.writeByte(this.gnssCnt);
		for (GpsCmdBean gpsCmdBean : this.gnssDataList) {
			gpsCmdBean.fillChannelBuffer(channelBuffer);
		}
	}
	...
	public void addGpsCmdBean(GpsCmdBean gpsCmdBean) {
		if (gpsCmdBean != null) {
			this.gnssDataList.add(gpsCmdBean);
			this.gnssCnt = (byte) this.gnssDataList.size();
		}
	}
}
